...
VmSubprovider.prototype.estimateGas = function (payload, end) {
  const self = this
  let lo = 0
  let hi = self._blockGasLimit
  if (!hi) {
    return end(new Error('VmSubprovider - missing blockGasLimit'))
  }

  const minDiffBetweenIterations = 1200
  let prevGasLimit = self._blockGasLimit
  doWhilst(
    function (callback) {
      ...
      self.runVm(payload, function (err, results) {
        const gasUsed = err ? self._blockGasLimit : ethUtil.bufferToInt(results.gasUsed)
        if (err || gasUsed === 0) {
          lo = mid
        } else {
          hi = mid
    ...
    function (err) {
      if (err) {
        ...
        hi = Math.floor(hi)
        const gasEstimateHex = rpcHexEncoding.intToQuantityHex(hi)
...
VmSubprovider.prototype.runVm = function (payload, cb) {
  ...
  const blockData = self.currentBlock
  const block = blockFromBlockData(blockData)
  const blockNumber = ethUtil.addHexPrefix(blockData.number.toString('hex'))

  // create vm with state lookup intercepted
  const vm = self.vm = createVm(self.engine, blockNumber, {
  ...
  const tx = new FakeTransaction(normalizedTxParams)
