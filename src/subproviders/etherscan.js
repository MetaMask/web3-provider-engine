...
EtherscanProvider.prototype.handleRequests = function (self) {
  ...
  for (let requestIndex = 0; requestIndex < self.times; requestIndex++) {
    const requestItem = self.requests.shift()
...
EtherscanProvider.prototype.handleRequest = function (payload, next, end) {
  const requestObject = { proto: this.proto, network: this.network, payload, next, end }
	  const self = this
...
function handlePayload (proto, network, payload, next, end) {
  switch (payload.method) {
    ...
    case 'eth_getLogs':
      return (function () {
        const payloadObject = payload.params[0]
        let txProcessed = 0
        const logs = []
        ...
        etherscanXHR(true, proto, network, 'proxy', 'eth_getBlockByNumber', {
          ...
          for (const transaction in blockResult.transactions) {
            etherscanXHR(true, proto, network, 'proxy', 'eth_getTransactionReceipt', { txhash: transaction.hash }, function (err, receiptResult) {
              if (!err) {
                logs.concat(receiptResult.logs)
              }
              txProcessed += 1
              if (txProcessed === blockResult.transactions.length) {
                end(null, logs)
              }
            })
          }
...
function etherscanXHR (useGetMethod, proto, network, module, action, params, end) {
  const uri = `${proto}://${network}.etherscan.io/api?${toQueryString({ module, action })}&${toQueryString(params)}`
