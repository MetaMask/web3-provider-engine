...
const EtherscanSubprovider = require('../../src/subproviders/etherscan')

test('etherscan eth_getBlockTransactionCountByNumber', function (t) {
  t.plan(3)

  const engine = new ProviderEngine()
  const etherscan = new EtherscanSubprovider()
...
test('etherscan eth_call', function (t) {
  ...
  const signature = Buffer.concat([sha3('getLatestBlock()', 256)], 4).toString('hex')
  const engine = new ProviderEngine()
  const etherscan = new EtherscanSubprovider()
  engine.addProvider(etherscan)
  engine.start()
