...
const SubscriptionSubprovider = require('../src/subproviders/subscriptions.js')
...
function subscriptionTest (label, opts, subscriptionPayload, afterInstall, subscriptionChangesOne, subscriptionChangesTwo) {
  ...
  const testMeta = {}
  const t = test(`subscriptions - ${label}`, function (t) {
    // subscribe
    // new block
    ...
    asyncSeries([
      ...
      (next) => {
        engine.sendAsync(createPayload(subscriptionPayload), function (err, _response) {
          ...
          const { method } = subscriptionPayload
